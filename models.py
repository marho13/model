...
def inception_v3(width, height, frame_count, lr, output=9, model_name = 'sentnet_color.model'):
    ...
    loss = fully_connected(pool5_7_7, output,activation='softmax')
    # b = input_data(shape=[500,9], name='b')
    # loss = tf.multiply(loss, b)
...
def inception_v3_unbalanced(width, height, frame_count, lr, output=9, model_name = 'sentnet_color.model'):
    network = input_data(shape=[None, width, height,3], name='input')
    conv1_7_7 = tf.keras.layers.Conv2D(network, 64, 7, strides=2, activation='relu', name = 'conv1_7_7_s2') #where does 64 and 7 come from? network, 64, 7, strides=2, activation='relu', name = 'conv1_7_7_s2'
    pool1_3_3 = tf.keras.backend.pool2d(conv1_7_7, 3,strides=2)
    pool1_3_3 = local_response_normalization(pool1_3_3)
    conv2_3_3_reduce = conv_2d(pool1_3_3, 64,1, activation='relu',name = 'conv2_3_3_reduce')
    conv2_3_3 = conv_2d(conv2_3_3_reduce, 192,3, activation='relu', name='conv2_3_3')
    conv2_3_3 = local_response_normalization(conv2_3_3)
...
# inception_v3_unbalanced(width=1920, height=1080, frame_count=500, lr=0.1)
